...
if '__main__' == __name__:
    ...
    parser.add_argument('--invert_rates_into_time', action='store_true')
    parser.add_argument('--draw_CI', action='store_true')
    o = parser.parse_args()
        ...
        for (df, linestyle) in zip(dfs, itertools.cycle(['-', '--', '-.', ':'])):
            for (lim, col), color in zip(itertools.product(o.replratio_cutoff[1:], columns), ['red', 'green', 'blue', 'grey', 'cyan', 'magenta', 'brown', 'purple', 'orange', 'black']):
                ...
                if col == 'degradationrate':
                    ...
                    dfR = df[(repl_ratio > lim_low) & (repl_ratio < lim_high) & (df[col+'_bootstrapfail%'] < max_fail)]
                if o.invert_rates_into_time:
                    try: timename = {'kon':'avg off time (1/kon)', 'koff':'avg on time (1/koff)', 'ksyn':'avg between tx during burst (1/ksyn)', 'degradationrate':'avg tx survival time (1/kd)'}[col]
                    except KeyError: continue
                    dfR['inv_'+col] = 1/df[col]
                    seaborn.kdeplot(x='inv_'+col, log_scale=True, data=dfR, linestyle=linestyle, color=color, label='%s, %i genes'%(timename, len(dfR)), bw_method=o.bw)
                    pyplot.xticks([365*24, 4*7*24, 7*24, 24, 8, 3, 1, 15/60, 4/60, 1/60, 15/60/60, 4/60/60], ['1y', '4wk', '1wk', '1day', '8h', '3h', '1h', '15min', '4min', '1min', '15s', '4s'])
                else:
                    ...
                    if o.draw_CI:
                        ...
                        CI95width = stats.gmean([v for v in repl_ratio if 1<v<1e10000])
                        geomSD = CI95width**(1/(2*stats.norm.ppf(0.975)))
                        geommean = stats.gmean([v for v in (df[col+'_bootstrapmedian'] if o.replmedians else df[col]) if 0<v<1e100000])
                        ypos_sd = 0.2
                        pyplot.plot([geommean/geomSD**0.5, geommean*geomSD**0.5], [ypos_sd, ypos_sd], linestyle='-', color=color)
                        pyplot.text(geommean, ypos_sd, 'SD', fontsize=8, color=color)
                        print(color, 'CI95width:', CI95width, 'geomSD:', geomSD, 'gmean:', geommean)
