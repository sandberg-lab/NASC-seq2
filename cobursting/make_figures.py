...
titles = ['new RNA (without allelic information)', 'new RNA (same allele)', 'new RNA (other allele)','Autosomal genes (same - other)', 'X-chromosome genes (same - other)', 'smallest genomic bin']
ylabels = ['Spearman correlation','Spearman correlation','Spearman correlation','Spearman correlation difference','Spearman correlation difference','allelic, control']
xlabels = ['Genomic distance (kbp)', 'Genomic distance (kbp)','Genomic distance (kbp)','Genomic distance (kbp)','Genomic distance (kbp)','allelic, cis']
ylims = [ [-1,1],[-1,1],[-1,1],[-0.6,0.6],[-0.6,0.6],[-1,1]]

data = [ results_na, results_nmean, results_xmean, results_nmx, results_nmx_X, [None,None] ]

xticks = ( [v+1 for v in range(5)], ['<%.2f' % (float(t)/1000000.) for t in separators])

red_circle = dict(markerfacecolor='red', marker='.', markeredgecolor='none', markersize= 2)

for subpl in range(6):
    plt.subplot(2,3,subpl+1)

    if subpl < 5:
        plt.boxplot(data[subpl], flierprops=red_circle)

    if subpl == 5:
        plt.scatter(results_nmean[0], results_xmean[0], s=1, edgecolor='none')
        
    plt.title(titles[subpl])
